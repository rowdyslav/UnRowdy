...
const SideBar = () => {
  ...
  return (
    <aside className={`border-primary border-r duration-300 flex flex-col ${isOpenSideBar ? 'w-48' : 'w-[100px]'}`}>
      <div className='border-b border-primary p-2 flex justify-start h-14 relative'>
        <img src={logo} alt='' width={50} height={50} className={`object-contain duration-300 ${!isOpenSideBar ? 'opacity-100' : 'opacity-0'}`}/>
        <h1 className={`text-3xl font-bold absolute -translate-x-[80] ${isOpenSideBar ? 'animation-fade-in' : 'opacity-0'}
        `}>UnRowdy</h1>
        <span
      ...
      <div className='p-2 flex-col flex justify-end flex-1 overflow-hidden'>
        <LogoutButton label={isOpenSideBar ? '' : <span className="material-symbols-outlined">logout</span>}/>
      </div>
    </aside>
  );
};
