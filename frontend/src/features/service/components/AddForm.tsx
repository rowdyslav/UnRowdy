import { ServiceFormSchema, type ServiceFormType } from '@/features/service/types/ServiceForm.schema.ts'
...
const AddForm = () => {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<ServiceFormType>({ resolver: zodResolver(ServiceFormSchema) })
      ...
      <form className='grid gap-y-5' onSubmit={handleSubmit(onSubmit)}>
        <div className='add-service-grid'>
          ...
          <div className='w-full'>
            <input {...register('name')} placeholder='Создание сайта' className='input py-2' />
        ...
        <div className='add-service-grid'>
          <h3 className='text-xl font-bold color-font w-1/2'>Изображение</h3>
          <div className='w-full center flex flex-col gap-y-1'>
            <label className='cursor-pointer'>
              <span className='px-4 py-2 button'>{file?.length ? file[0].name : 'Загрузить'}</span>

              <input type='file' accept='image/*' className='hidden' {...register('image')} />
            </label>
            <p className='text-center text-red-400'>{errors?.image?.message}</p>
          </div>
        </div>

        <button type='submit' className='mt-5 button w-1/2 justify-self-center'>
          Создать услугу
        </button>
      </form>
    </div>
